...
pub fn part2_naive(input: &str) -> usize {
    ...
    for &(row, col) in &empty_cells {
        // Place an obstacle at (row, col)
        let original_char = grid[row][col];
        // To mutate the grid, convert to mutable grid
        // Clone once outside the loop
        // Alternatively, use a mutable grid
        // Here, we'll use a mutable copy
        // Since grid is large, it's better to create a mutable grid once
        // and reuse it
        // Therefore, create a mutable grid
        // outside the loop
        // To avoid modifying the original grid, we need a mutable copy
        // So, let's create a mutable grid once
        // and reset it inside the loop
        // However, Rust does not allow mutable and immutable borrows simultaneously
        // Therefore, we'll restructure the code

        // To avoid multiple clones, we can create a mutable grid outside
        // Clone the grid once
