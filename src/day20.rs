    ...
    *VISITED.get_unchecked_mut(end_pos) = TRUE;
    ...
    loop {
        ...
        for i in 0..queue.1 {
            ...
            expand(left(coords), next_cost, input, next);
            expand(right(coords), next_cost, input, next);
            expand(up(coords), next_cost, input, next);
            expand(down(coords), next_cost, input, next);
        }
        (*queue).1 = 0;
    }
}
